...
Animation.slideInFromRight = function(element, options) {
    ...
    requestAnimationFrame(function() {
        requestAnimationFrame(function() {
            element.style.transition = "transform 0.15s " + Animation.EASING;
            element.style.transform = "translateX(0)";

            if (typeof doneCallback === "function") {
                setTimeout(doneCallback, Animation.DURATION_MS);
            }
        });
    });
...
/**
 * Đóng submenu: trượt sang trái (translateX 0 → -100%).
 * Cặp với slideInFromLeft.
 *
 * @param {Element} element - Phần tử cần animate
 * @param {Object} [options] - doneCallback, removeProperties
 * @returns {boolean}
 */
...
Menu.Context.removeChildrenListeners = function ( newEl ) {
    ...
    allChildren.forEach(child => {
        ...
        // Finalizer.Optimize.lockAriaExpanded( clone );
...
Menu.Context.Horizontal.refreshHeaderTitle = function(menu) {
    ...
    var openedLevel3 = Menu.Context.Horizontal.getOpenedSubmenu(menu, "3");
    if (openedLevel3 && openedLevel3.li) {
        Menu.Context.Horizontal.updateHeaderTitleFromMenuItem(openedLevel3.li);
        return;
    }
...
Menu.Context.Horizontal.applyOpenAnimation = function (menuItem, submenuElement, menuKind, mode) {
    ...
    var config = {
        ...
        "3": { expandClass: "menu-expand-level2", displayValue: "inline-block", initialStyles: { width: "100%" } }
    };
    ...
    var doneCallback = function() {
        ...
        if (Menu.Context.Horizontal && typeof Menu.Context.Horizontal.updateHeaderTitleFromMenuItem === "function") {
            Menu.Context.Horizontal.updateHeaderTitleFromMenuItem(menuItem);
        }

        // Khi mở level 3: lưu scroll hiện tại của level 2, reset về top + khoá scroll level 2, chỉ cho panel level 3 scroll
        if (menuLevel === "3" && submenuElement) {
            var level2Ul = submenuElement.closest ? submenuElement.closest('ul.children[menulevel="2"]') : null;
            if (level2Ul) {
                if (typeof level2Ul._naviPrevScrollTop === "undefined") {
                    level2Ul._naviPrevScrollTop = level2Ul.scrollTop || 0;
                }
                // Đưa level 2 về top để tránh cảm giác bottom của level 3 “dính” đúng item được chọn
                level2Ul.scrollTop = 0;
                level2Ul.classList.add("navi-level2-scroll-locked");
            }
        }
    ...
    /* Mobile: slideInFromRight. Desktop leftright + panel trái: slideInFromLeft. */
...
Menu.Context.Horizontal.closeLevel3 = function(menu) {
    ...
    // Khôi phục scroll level 2 NGAY LẬP TỨC (khi level 3 vẫn đang che),
    // để người dùng không thấy content level 2 nhảy.
    var level2Ul = ul.closest ? ul.closest('ul.children[menulevel="2"]') : null;
    if (level2Ul && typeof level2Ul._naviPrevScrollTop !== "undefined") {
        level2Ul.scrollTop = level2Ul._naviPrevScrollTop || 0;
        delete level2Ul._naviPrevScrollTop;
    }

    Menu.Context.Horizontal.applyCloseAnimation(ul, "level3", function() {
        // Mở lại scroll cho level 2 (nếu có)
        var level2UlDone = ul.closest ? ul.closest('ul.children[menulevel="2"]') : null;
        if (level2UlDone) {
            level2UlDone.classList.remove("navi-level2-scroll-locked");
        }
...
// Menu.Context.Horizontal.applyCloseAnimation
    ...
    document.addEventListener("click", function(event) {
        ...
        if (
            ...
            if (
                ...
                console.log("Finalizer.Event.handleTouchOutsideAllMenus: Close submenu, back to level 2");
                ...
                console.log("Finalizer.Event.handleTouchOutsideAllMenus: Close all submenus");

                document.querySelectorAll('.naviman_app ul li ul.children').forEach((item) => {
                    item.style.display = "none";
                });
                // Nếu chưa load được JS này sẽ lỗi
...
Finalizer.Event.handleTouchBackButtonOnSlideMenuHorizontal = function() {
    document.addEventListener("click", function(event) {
        // Kiểm tra nút back (chỉ xử lý nếu là hamburger_back hoặc back-icon)
