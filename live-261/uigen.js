...
navihelper.getShopInfo = function(shop, token) {     
    ...
    shopinfo = fetch(api_shopinfo_url + "?v=" + Math.random()) 
    ...
    // Bước 4: Truyền vào biến windowVar để dùng lại (Bước này thật ra ko cần thiết vì có thể dùng luôn session)    
    navihelper.windowVar.set('shopinfo', shopinfo);    
...
window.navisession = window.navisession || new window.NaviSessionStorage(60); // TTL = 60s
...
var getMenuKindByHtml = function( embed_id ) {
    ...
    if (menuEl) {
        ...
        if (menuEl.classList.contains("TABBAR"))
            return NAVIGLOBAL['MENU_KINDS']['STICKY_TABBAR'];
        if (menuEl.classList.contains("FAB"))
...
Helper.Animation.applySlidemenu_SubmenuOpen = function (menuItem, submenuElement, menuKind, mode) {
    if (!submenuElement) return false;

    if (menuKind != NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE'])
        return false;

    var runAnimationAndUpdateBack = function() {
        requestAnimationFrame(function() {
            submenuElement.style.transition = "margin-left 0.15s cubic-bezier(0.4, 0.0, 0.2, 1), opacity 0.15s ease-in-out";
            submenuElement.style.marginLeft = "0px";
            submenuElement.style.opacity = "1";
            var menu = menuItem.closest ? menuItem.closest(".naviItem.CONTEXT_SLIDE") : null;
            if (typeof window.naviman_updateHamburgerBackVisibility === "function")
                window.naviman_updateHamburgerBackVisibility(menu);
        });
    };

    if (mode == "level2") {
        const menuLevel = submenuElement.getAttribute('menulevel');
        if (menuLevel !== "2") return false;

        submenuElement.style.marginLeft = "50px";
        submenuElement.style.opacity = "0";
        if (submenuElement.style.display == "" || submenuElement.style.display == "none")
        ...
        runAnimationAndUpdateBack();
        return true;
    }

    if (mode == "level3") {
        submenuElement.style.width = "100%";
        submenuElement.style.transform = "translateX(50px)";
        submenuElement.style.opacity = "0";
        ...
        if (menuKind == NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE'])
            displayElement(submenuElement, true, "inline-block");
        else
            displayElement(submenuElement, true, "block");

        requestAnimationFrame(function() {
            requestAnimationFrame(function() {
                submenuElement.style.transition = "transform 0.15s cubic-bezier(0.4, 0.0, 0.2, 1), opacity 0.15s ease-in-out";
...
Helper.Animation.applySlidemenu_SubmenuClose = function (menuItem, submenuElement, menuKind, mode, doneCallback) {
    if (!submenuElement) return false;

    var done = function() {
        if (typeof doneCallback === "function") doneCallback();

        const menu = menuItem && menuItem.closest ? menuItem.closest(".naviItem.CONTEXT_SLIDE") : null;
        if (typeof window.naviman_updateHamburgerBackVisibility === "function")
            window.naviman_updateHamburgerBackVisibility(menu);
    };
    ...
    if (mode == "level2") {
        ...
        submenuElement.style.transition = "margin-left 0.15s cubic-bezier(0.4, 0.0, 0.2, 1), opacity 0.1s ease-out";
        ...
        setTimeout(() => {
            ...
            submenuElement.style.removeProperty("margin-left");
...
Menu.Sticky.lockPageScrollingTabBar = function (menuKind, isLock) {
    ...
    navidebug.log("LOCK PAGE SCROLL... FOR TAB BAR (MOBILE ONLY)");
...
Menu.Context.checkTriggerIDClass = function(isDisplayed, naviItem ) { 
    ...
    if( naviItem["data"]["setting"]['publishTriggerIDClass'] == "" )
      ...
      var elsArray = Helper.String.stringToArray(naviItem["data"]["setting"]['publishTriggerIDClass']);
        ...
        elsArray.forEach(function(elsIdClass) {
            navidebug.log(naviItem["embed_id"] + " added trigger event to: " + elsIdClass);
            ...
            els.forEach(function(el) {
                ...
                if (localStorage.getItem("__isDebug") === "true") 
                    navidebug.log("Remove children successfully");
  ...
  navidebug.log( naviItem["embed_id"] + " is always shown. The cause is: CONTEXT_SLIDE" );
}
...
var hideAllLevel3Items = function () {
    console.log("hideAllLevel3Items");
...
var showLevel3Items_Common = function (menuItem, isNaviSection, menuKind, embed_id, event) {
    ...
    if (Menu.Context.isSlideMenuHorizontal(menuKind, embed_id)) {
        console.log("showLevel3Items_Common - isSlideMenuHorizontal");
        isInnerExpand = false;
    }    
    ...
    */
    ...
    if (ulChildrent.style.display == "none" || ulChildrent.style.display == "" || ulChildrent.style.display == "initial") {
        ...
        if (menuKind == NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE'])
            Helper.Animation.applySlidemenu_SubmenuOpen(menuItem, ulChildrent, menuKind, "level3");
        ...
        if (menuKind == NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE']) {
            const menu = menuItem.closest ? menuItem.closest(".naviItem.CONTEXT_SLIDE") : null;
            if (typeof window.naviman_updateHamburgerBackVisibility === "function")
                window.naviman_updateHamburgerBackVisibility(menu);
        }
    }
    ...
    if (isInnerExpand) {
        ...
        console.log("showLevel3Items_Common - isInnerExpand: Là dạng inner expand (tabbar, mobile mega menu..)");

        ulChildrent.style.display = "block";
        var ulLength = Helper.HTML.getULLength(ulChildrent);
        ...
        console.log("showLevel3Items_Common - là dạng expand, trên desktop, slide menu horizontal");
        ulChildrent.style.display = "block";
...
var showLevel3Items_Fix_Expand =  function (menuKind, ulChildrent) {
    if (menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] ||
        menuKind == NAVIGLOBAL['MENU_KINDS']['CONTEXT_SLIDE']) {
            console.log("showLevel3Items_Fix_Expand - menuKind:", menuKind);
...
var showLevel2Items = function (menuItem, menuKind, embed_id, event) {
    ...
    if (menuItem != null) {
        if (!is_showing) {            
            ...
            // Add animation for level 2 submenu
            var submenuElement = menuItem.querySelector('ul.children');
            lastSlidemenuOpenTime[embed_id] = Date.now();

            if (!Helper.Animation.applySlidemenu_SubmenuOpen(menuItem, submenuElement, menuKind, "level2")) {
                menuItem.classList.add("menu-expand");
                menuItem.classList.add("menu-expand-level1");
            }
            ...
            var closeDropDown = function () {
                ...
                Helper.hideNaviOverlay();
                const menu = document.getElementById(embed_id);
                if (typeof window.naviman_updateHamburgerBackVisibility === "function")
                    window.naviman_updateHamburgerBackVisibility(menu);
            }
...
if (!window._navimanClickListenerAttached) {
    ...
    var naviman_slideHorizontal_getOpenedSubmenu = function(menu, menuLevel) {
        if (!menu) return;
        if (!menu.classList || !menu.classList.contains("CONTEXT_SLIDE")) return;
        if (menu.getAttribute("ui") != "slide-horizontal") return;
    ...
    var naviman_slideHorizontal_applyCloseAnimation = function(submenuElement, mode, doneCallback) {
        if (!submenuElement) return false;

        // console.log("naviman_slideHorizontal_applyCloseAnimation", submenuElement, mode);
        ...
        if (mode == "level2") {
            submenuElement.style.transition = "opacity 0.15s ease-in-out";
            requestAnimationFrame(() => {
                submenuElement.style.opacity = "0";
            });
            setTimeout(() => {
    ...
    var naviman_slideHorizontal_closeLevel3 = function(menu) {
        if (!menu) return;
        if (!menu.classList || !menu.classList.contains("CONTEXT_SLIDE")) return;
        if (menu.getAttribute("ui") != "slide-horizontal") return;

        // console.log("naviman_slideHorizontal_closeLevel3", menu);

        const opened = naviman_slideHorizontal_getOpenedSubmenu(menu, "3");
        ...
        naviman_slideHorizontal_applyCloseAnimation(ul, "level3", function() {
            if (typeof window.naviman_updateHamburgerBackVisibility === "function")
                window.naviman_updateHamburgerBackVisibility(menu);
        });
    ...
    var naviman_slideHorizontal_closeLevel2 = function(menu) {
        if (!menu) return;
        if (!menu.classList || !menu.classList.contains("CONTEXT_SLIDE")) return;
        if (menu.getAttribute("ui") != "slide-horizontal") return;

        // console.log("naviman_slideHorizontal_closeLevel2", menu);

        const opened = naviman_slideHorizontal_getOpenedSubmenu(menu, "2");
        if (!opened) return false;
        ...
        naviman_slideHorizontal_applyCloseAnimation(ul, "level2", function() {
            ...
            if (typeof Menu !== "undefined" && Menu.Common && typeof Menu.Common.removeTopZindex === "function")
                Menu.Common.removeTopZindex(li, menuKind, embed_id);

            if (typeof Helper !== "undefined" && typeof Helper.hideNaviOverlay === "function")
                Helper.hideNaviOverlay();
    ...
    ********************************************************/   
    window.addEventListener("load", function() {
        ...
        if (window.location.href.includes("simulator.php")) {
            // navidebug.log( "Shopify info: In simulator mode " );
        } else {
            if( !window.Shopify )
    ...
    document.addEventListener("click", function(event) {
        ...
        if (isCloseBtn || isOverlay) {
            document.querySelectorAll(".naviItem.CONTEXT_SLIDE").forEach((item) => {
                ...
                if (typeof window.naviman_updateHamburgerBackVisibility === "function") {
                    window.naviman_updateHamburgerBackVisibility(item);
                } else {
            ...
            // navidebug.log("Slide menu closed by click on close button or overlay");
        }
    });    

    /*******************************************************
    Back button cho CONTEXT_SLIDE ui=slide-horizontal:
    - Đóng submenu đang mở (level 3 -> level 2 -> level 1)
    *******************************************************/
    document.addEventListener("click", function(event) {
        const isBackBtn = event.target.closest(".naviItem .hamburger_back, .naviItem .back-icon");        
        ...
        // Chặn trigger click của menu item phía sau
        if (event) {
        ...
        if (naviman_slideHorizontal_closeLevel3(menu)) return;
        if (naviman_slideHorizontal_closeLevel2(menu)) return;
    });
...
var BodyLocker = (function() {
  var backup = null;
  var retryTimeouts = null;

  /** Kiểm tra Navi.Setting.lockBodyScroll_fixBodyPosition có bật không */
  function isFixBodyPosition() {
    return typeof Navi !== "undefined" &&
      Navi.Setting &&
      Navi.Setting.lockBodyScroll_fixBodyPosition === true;
  }

  /** Kiểm tra body đã bị lock chưa (overflow hidden) */
  function isBodyLocked() {
    var html = document.documentElement;
    var body = document.body;
    return body.style.overflow === "hidden" && html.style.overflowY === "hidden";
  }

  /** Áp dụng styles lock (overflow hidden, position fixed nếu cần). Chỉ ghi đè khi chưa lock. */
  function applyStyles(fixBodyPosition, isRetry) {
    ...
    if (fixBodyPosition)
      body.style.position = "fixed";
    if (isRetry) console.log("BodyLocker - ghi đè lại (đã bị overwrite)");
  }

  /** Khôi phục styles từ backup */
  function restoreStyles(backupData, fixBodyPosition) {
    var html = document.documentElement;
    var body = document.body;
    body.style.overflow = backupData.body_overflow || "";
    html.style.overflowY = backupData.doc_overflowY || "";
    if (fixBodyPosition)
      body.style.position = backupData.body_position || "";
  }

  /** Kiểm tra overlay .naviman_app_overlay_global có đang hiển thị không */
  function isOverlayVisible() {
    var overlay = document.querySelector(".naviman_app_overlay_global");
    if (!overlay) return false;
    var style = window.getComputedStyle(overlay);
    return style.display !== "none" && style.visibility !== "hidden";
  }

  /** Lên lịch retry: mỗi intervalMs, nếu overlay vẫn hiện và body bị overwrite thì gọi lại applyFn */
  function scheduleRetries(applyFn, retryCount, intervalMs) {
    cancelRetries();
    retryTimeouts = [];
    for (var i = 1; i <= retryCount; i++) {
      (function(retryIndex) {
        var tid = setTimeout(function() {
          if (isOverlayVisible() && !isBodyLocked()) applyFn();
        }, retryIndex * intervalMs);
        retryTimeouts.push(tid);
      })(i);
    }
  }

  /** Hủy tất cả retry timeouts */
  function cancelRetries() {
    if (retryTimeouts) {
      retryTimeouts.forEach(function(id) { clearTimeout(id); });
      retryTimeouts = null;
    }
  }

  /** Lock scroll: lưu backup, apply styles, schedule retries */
  function lock() {
    var html = document.documentElement;
    var body = document.body;
    var fixBodyPosition = isFixBodyPosition();

    if (fixBodyPosition)
      navidebug.log("Navi.Setting.lockBodyScroll_fixBodyPosition", Navi.Setting.lockBodyScroll_fixBodyPosition);
  ...
  function unlock() {
    cancelRetries();
    if (backup) {
      if (isBodyLocked()) console.log("BodyLocker - unlock, restore");
      restoreStyles(backup, isFixBodyPosition());
      backup = null;
    }
  }
...
/** API tương thích: lock (true) hoặc unlock (false) */
...
(() => {
  ...
  const TEXT = {
    ...
    modalDescription: [
      "<div>This is a support tool provided by <a href=\"https://naviplus.io\" target=\"_blank\">Navi+ | Web Menu Builder </a>.",
      "It helps you identify and test \"CSS selectors\" on your website for use in the following scenarios:</div>",
      "<div>- <a href='https://help.naviplus.io/docs/usage/explore-common-menus/#2-mega-menu-desktop---extremely-popular' target=_blank>Mega Menu</a>,",
      "<a href='https://help.naviplus.io/docs/usage/explore-common-menus/#6-grid-menu-' target=_blank>Grid Menu</a>:",
      "Place the menu before, after, or replace the selected \"CSS selector\"</div>",
      "<div>- <a href='https://help.naviplus.io/docs/usage/explore-common-menus/#4-slide-menu-hamburger-menu' target=_blank>Slide Menu</a>:",
      "Open the menu when the element matched by a \"CSS selector\" is tapped or clicked</div>",
      "<hr style='margin: 8px 0px;border: dashed 1px #999;'>",
      "<b style='font-size:16px'>How to use?</b>",
      "<div style='padding-top: 4px'>Hover over the target area to view its \"CSS selector\",",
      "then save and apply it to the menu settings. </div>",            
      "<div style='padding-top:8px'><a target=_blank href='https://help.naviplus.io/docs/usage/debug-mode-find-css-selectors'>",
      "- View the guide to using Navi+ Debug</a></div>",
      "<div style='padding-top:4px'><a target=_blank href='https://help.naviplus.io/docs/usage/debug-mode-find-css-selectors/#12-find-css-selector-on-mobile'>",
      "- How to find CSS Selectors on mobile?</a></div>",
      "<hr style='margin: 8px 0px;border: dashed 1px #999;'>"
    ].join(""),
    ...
    inspectorCopyHintMac: "To copy: [⌘ + C] | Move to parent level: [↑/←], [↓/→] child",
    inspectorCopyHintWindows: "To copy: [Ctrl + C] | Move to parent level: [↑/←], [↓/→] child",
  ...
  const syncInspectorWithDebugMode = () => {
    if (isDebugModeEnabled()) enableInspector();
  ...
  const createDebugUI = () => {
    ...
    status.textContent = TEXT.debugStatusOn;

    const offBtn = document.createElement("button");
    offBtn.type = "button";
    offBtn.setAttribute("data-part", "off");
    offBtn.textContent = TEXT.debugTurnOff;
    ...
    headerRight.appendChild(offBtn);
    ...
    p1.innerHTML = TEXT.modalDescription;
    ...
    p4.appendChild(document.createTextNode(TEXT.disableLabel));
    ...
    offBtn.addEventListener("click", () => {
      const nextUrl = `${window.location.origin}${window.location.pathname}${window.location.search}#navidebug-off`;
  ...
  const createInspector = () => {
    ...
    const onKeyDown = async (e) => {
      ...
      const isCopyKey = String(e.key || "").toLowerCase() === "c";
      if (!isCopyKey) return;

      const wantsCopy = isMac ? e.metaKey : e.ctrlKey;
      if (!wantsCopy) return;
...
var initNaviplusApp = function (e) {
    ...
    function getJSONData(shopinfo, token) {
        ...
        // Bước 2: Thử lấy từ sessionStorage (navisession) trước để tránh fetch lại
        var cachedJson = window.navisession.get(api_url);
        if (cachedJson) {
            navihelper.reportSteps("---> HAHA! Lấy JSON data từ sessionStorage (navisession) cho URL: " + api_url);
        ...
        .then((response) => {
            ...
            return response.json(); // Browser parse JSON trực tiếp từ stream
        })
        .catch(() => {
        ...
        .then((json) => {
            // Bước 5: Lưu vào sessionStorage để lần sau khỏi fetch
            window.navisession.set(api_url, json);
    ...
    navihelper.getShopInfo(shop, section_setting['token']).then((shopinfo) => {
        getJSONData(shopinfo, token);
    });
