...
var applyHamburgerWidthSettings = function ( data, embed_id ) {
    ...
    if (css) {
        // hamburgerPosition == 7 (Fix on left): menu luôn visible → inject CSS ngay lập tức (không qua setTimeout)
        // để tránh flash 360px trước khi width cấu hình được áp dụng.
        var _isFixedAlwaysVisible = parseInt(setting["hamburgerPosition"] || 1) === 7;
        if (_isFixedAlwaysVisible) {
            (document.head || document.body).insertAdjacentHTML('beforeend', '<style>' + css + '</style>');
        } else {
            Helper.HTML.addLoadedFSAtBodyEnd('<style>' + css + '</style>', 22);
        }
    }
...
(() => {
  ...
  const TEXT = {
    ...
    modalDescription: [
      ...
      "<b style='font-size:15px;display:block;padding-top:12px'>How to use?</b>",
      ...
      "<span style='padding-top:8px'><a target=_blank href='https://help.naviplus.io/docs/usage/debug-mode-find-css-selectors'>",
      "View detail</a></span> | ",
      "<span style='padding-top:4px'><a target=_blank href='https://help.naviplus.io/docs/usage/debug-mode-find-css-selectors/#12-find-css-selector-on-mobile'>",
      "How to find CSS Selectors on mobile?</a></span>",
