...
#${modalId} [data-part="close"] {
    ...
    const _tryMenusHiddenKeys = new Set();
    ...
    const renderTryMenuEditGallery = async (key, els, state) => {
      ...
      if (db && Array.isArray(db[tabKey])) {
        list = list.concat(db[tabKey].filter((t) => t && t.source_embed_id && t.source_embed_id !== TRY_MENUS_EMBEDS[key]));
      }
      ...
      let lastTier;
      list.forEach((t) => {
        if (t.tier_label && t.tier_label !== lastTier) {
          const tierEl = document.createElement("div");
          tierEl.className = "navi-edit-tier";
          tierEl.textContent = t.tier_label;
          els.gallery.appendChild(tierEl);
          lastTier = t.tier_label;
        }
