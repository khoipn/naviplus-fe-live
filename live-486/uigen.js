...
navihelper.getShopInfo = async function(shop, token) {     
    ...
    var fetchPromise = (async function() {
        var shopinfo;
        try {
            // TASK00063. Begin — Cache-buster theo bucket 10s (đồng bộ với sessionStorage TTL) thay cho Math.random() để edge/browser cache có thể hit
...
Helper.Env.isInvalidEmbed = function( section_setting ) {
    ...
    // TASK00177. Begin — Wix: shop=siteId không phải domain thật → embed luôn hợp lệ (skip domain check)
...
Menu.Context.createTriggerOverlay = function(el, embedId, selector, openHandler) {
    ...
    /* Idempotent: nếu trigger overlay sibling đã tồn tại → reposition + cập nhật _triggerPairs[i].el
     * el có thể đã bị DOM replace bởi theme → cập nhật lại con trỏ để reposition đúng. */
    var sib = el.previousElementSibling;
    while (sib) {
        if (sib.classList.contains(OVERLAY_TRIGGER_CLASS) && sib.getAttribute(DATA_TRIGGER_EMBED) === embedId) {
            Menu.Context._repositionTriggerOverlay(el, sib);
            for (var i = 0; i < Menu.Context._triggerPairs.length; i++) {
                if (Menu.Context._triggerPairs[i].overlay === sib) {
                    Menu.Context._triggerPairs[i].el = el;
                    break;
                }
            }
            return;
    ...
    overlay.style.cssText = "display:block;position:absolute;z-index:9999;background:transparent;cursor:pointer;";
...
Menu.Context.bindTriggerToElement = function(el, embedId, elsIdClass, isReBind) {
    ...
    Animation.flashTriggerBound(el);
...
Menu.Context._runTriggerRebindCheck = function() {
    ...
    // Reposition trigger + protect overlays (layout có thể đã thay đổi)
    ...
    // Tìm element mới hoặc bị replace (chưa có DATA_TRIGGER_BOUND) → bind lại
...
Menu.Context._startTriggerMutationObserver = function() {
    ...
    Menu.Context._triggerObserver = new MutationObserver(function() {
        ...
        Menu.Context._triggerRebindDebounceTimer = setTimeout(Menu.Context._runTriggerRebindCheck, 50);
    });
...
/*        addHtml += cssNaviPrefix + ' ul li.item .icon, ' + cssNaviPrefix + ' ul li.item .image { height: ' + (21 + (setting['spaceTextIcon'] - 2)) + 'px; } ';*/
    ...
    if (setting['desktopHoverStyle'] !== 'underline') {
        ...
        if (_bgColor || _txtColor || _txtAnim === 'none' || _txtAnim === 'bounce-x') {
            ...
            if (_bgColor) {
                addHtml += cssNaviPrefix + ' ul li.item:hover:not(.kind-group-title), '
                         + cssNaviPrefix + ' ul li.child:hover:not(.kind-group-title) { background: ' + _bgColor + ' !important; } ';
            }
            if (_txtColor) {
                ...
                addHtml += cssNaviPrefix + ' ul li.item:hover > .inner .name, '
                         + cssNaviPrefix + ' ul li.child:hover > .inner .name { color: ' + _txtColor + ' !important; } ';
            }
            ...
            if (_txtAnim === 'none') {
                addHtml += cssNaviPrefix + ' ul li.item > .inner .name:hover, '
                         + cssNaviPrefix + ' ul li.child > .inner .name:hover { animation: none !important; } ';
            } else if (_txtAnim === 'bounce-x') {
                addHtml += '@keyframes naviBounceSideways { 0%, 100% { transform: translateX(0); } 50% { transform: translateX(-2px); } } ';
                addHtml += cssNaviPrefix + ' ul li.item > .inner .name:hover, '
                         + cssNaviPrefix + ' ul li.child > .inner .name:hover { animation: naviBounceSideways 0.2s ease !important; } ';
            }
            addHtml += '} ';
        }
    }
    ...
    if (setting['desktopHoverStyle'] === 'underline') {
        ...
        /* linkto="#" = placeholder link, không navigate → skip underline.
           linkto="" = không có link nhưng vẫn muốn có hiệu ứng → giữ nguyên. */
        var _notHash        = ':not([linkto="#"])';
        var _nameEl         = cssNaviPrefix + ' ul li.item > .inner .name, '                                          + cssNaviPrefix + ' ul li.child > .inner .name';
        var _nameAfter      = cssNaviPrefix + ' ul li.item' + _notHash + ' > .inner .name::after, '                  + cssNaviPrefix + ' ul li.child' + _notHash + ' > .inner .name::after';
        ...
        var _hoverNameAfter = cssNaviPrefix + ' ul li.item' + _notHash + ':hover:not(:has(li.child:hover)) > .inner .name::after, ' + cssNaviPrefix + ' ul li.child' + _notHash + ':hover > .inner .name::after';
        addHtml += '@media screen and (min-width:769px) { ';
        /* animation: none !important scoped theo embed_id — tránh xung đột @keyframes global giữa các menu. */
        addHtml += cssNaviPrefix + ' ul li.item > .inner .name:hover, '
                 + cssNaviPrefix + ' ul li.child > .inner .name:hover { animation: none !important; } ';
        // Xóa background hover mặc định — underline thay thế cho tất cả items
        addHtml += cssNaviPrefix + ' ul li.item:hover:not(.kind-group-title), '
                 + cssNaviPrefix + ' ul li.child:hover:not(.kind-group-title) { background: transparent !important; } ';
        ...
        if (_animType === 'ltr') {
            ...
            addHtml += _nameAfter      + ' { ' + _afterBase + 'opacity: 0; transition: opacity 0.25s ease; } ';
    ...
    return  false;
        ...
        var Draw_toEmbedIDCSSSelector = function(naviItem, embed_id, shop, data, isNaviSection, section_setting, menuKind, menuKindClass, response, isPublishToPlace, naviman_app ) {
            ...
            if (naviman_appItem) {
                ...
                (function() {
                    var _revealAll = function() {
                        naviman_appItem.querySelectorAll('i.navi-skeleton, .material-symbols-outlined.navi-skeleton').forEach(function(el) {
                            el.classList.remove('navi-skeleton');
                        });
                    };

                    /* fonts.ready: chờ Remix (và mọi font đã track trước insertAdjacentHTML) */
                    if (document.fonts && document.fonts.ready) {
                        document.fonts.ready.then(_revealAll);
                    }

                    /* Material Symbols: chỉ chờ nếu menu có ms: icons.
                       Chuỗi cssReady → fonts.load() đảm bảo fonts.load chỉ gọi SAU KHI <link> đã load xong
                       (tránh race condition fonts.load resolve ngay khi CSS chưa được parse). */
                    var _hasMSIcons = naviman_appItem.querySelector('.material-symbols-outlined.navi-skeleton') !== null;
                    if (_hasMSIcons) {
                        var _t = setTimeout(_revealAll, 3000);
