 ...
 * https://cdn.jsdelivr.net/gh/khoipn/naviplus-fe-live@main/live-206/uigen.min.js
 * THÌ LẤY PHIÊN BẢN THEO CÁI NÀY **/
// var uigenJsScriptUrl = "https://cdn.jsdelivr.net/gh/khoipn/naviplus-fe-live@main/live-206/uigen.min.js";
// var versionMatch = uigenJsScriptUrl.match(/\/([^\/]+)\/uigen\.min\.js$/);
// if (versionMatch) {
//     naviman_version = versionMatch[1];
//     navidebug.log(naviman_version);
// }


/********************************************/
// Lấy URL của script này, có fallback nếu currentScript không có
/********************************************/
var uigenJsScriptUrl = null;
var uigenJsScriptElements = null;

try {
    // Thử lấy từ currentScript (chỉ hoạt động khi script chạy synchronously)
    if (document.currentScript && document.currentScript.src) {
        uigenJsScriptUrl = document.currentScript.src;
        uigenJsScriptElements = document.currentScript;
    } else {
        console.error("uigen.min.js - currentScript is not found, fallback to find script tag");
        // Fallback: Tìm script tag có chứa 'uigen.js' hoặc 'uigen.min.js'
        const scripts = document.getElementsByTagName('script');
        for (let i = scripts.length - 1; i >= 0; i--) {
            const src = scripts[i].src || '';
            if (src.includes('uigen.js') || src.includes('uigen.min.js')) {
                uigenJsScriptUrl = src;
                uigenJsScriptElements = scripts[i];
                break;
            }
        }
    }
} catch (e) {
    console.warn('⚠️ Cannot get uigenJsScriptUrl (uigen.min.js)):', e);
}
...
var cdn_golive_cloudflare_page = 'https://live.naviplus.app/live'; // Dùng cho LIVE (Cloudflare Pages)

var naviplusCss = uigenJsScriptUrl.replace('.js', '.css'); // cdn_golive_cloudflare_page + '/'+ naviman_version +'/uigen.min.css';
...
/**********************************************************************************/
// if (uigenJsScriptUrl && uigenJsScriptUrl.indexOf('uigen.js.php') !== -1) {
...
/**
 * Lấy thông tin shopinfo từ server (theo CDN) và cache vào biến toàn cục để dùng lại.
 * 
 * Luồng lấy shopinfo:
 * - Nếu đã có shopinfo (đã fetch trước đó), trả về dữ liệu ngay (Promise resolved).
 * - Nếu chưa có shopinfo nhưng đang có 1 request fetch shopinfo (promise), dùng lại promise này.
 * - Nếu chưa có gì:
 *      + Gửi request GET tới file .info.json ở CDN tuỳ theo shop.
 *      + Cache lại promise để các lần gọi đồng thời chỉ request 1 lần.
 *      + Khi thành công thì cache data shopinfo, khi lỗi thì trả lỗi để xử lý ngoài.
 * 
 * @param {string} shop - Tên shop cần lấy thông tin
 * @returns {Promise<object>} - Promise resolve ra object shopinfo lấy từ CDN
 */

navihelper.getShopInfo = function(shop) {
    // Đảm bảo biến promise trên window luôn tồn tại nhất quán
    if (!navihelper.windowVar.isExisted('shopinfoPromise'))
        navihelper.windowVar.set('shopinfoPromise', null);    

    // Nếu đã có shopinfo, trả về Promise resolve ngay với dữ liệu
    if (navihelper.windowVar.isExisted('shopinfo')) {
        return Promise.resolve(navihelper.windowVar.get('shopinfo'));
    }

    // Nếu đang lấy promise shopinfo thì dùng lại
    let shopinfoPromise = navihelper.windowVar.get('shopinfoPromise');
    if (!shopinfoPromise) {
        // Tạo request fetch shop info mới
        var api_shopinfo_url = naviplusCDNJson + "/" + shop.replace(".myshopify.com", '') + ".info.json" + "?v=" + Math.random();    
        // navidebug.log("[DATA]: ShopInfo (NO CACHE): " + api_shopinfo_url);

        navihelper.reportSteps("DATA SHOPINFO URL: " + api_shopinfo_url);

        shopinfoPromise = fetch(api_shopinfo_url)
            .then((response) => response.json())
            .then((shopinfo) => {
                navihelper.windowVar.set('shopinfo', shopinfo);
                navidebug.log(shopinfo);
                return shopinfo;
            })
            .catch((error) => {
        ...
        var callDrawLoop = function(
            ...
            const timer = setInterval(() => {
                ...
                if (elapsed > durationMs) {
                    clearInterval(timer);
                    navidebug.log('⏱️ Stop loop after', durationMs, 'ms');
                    return;
                }
        ...
        var Draw_toEmbedIDCSSSelector = function(naviItem, embed_id, shop, data, isNaviSection, section_setting, menuKind, menuKindClass, response, isPublishToPlace, naviman_app ) {
            ...
            if (naviman_appItem) {
                ...
                if (!Helper.Env.isBackendMode()) {
                    ...
                    setTimeout(() => logGAEvent("navi_visit", { navi_embed_id: naviItem["embed_id"] }), 300);
...
var showLevel2Items = function (menuItem, menuKind, embed_id) {
    ...
    if( desktopHover ) {
        ...
        if (now - lastShowLevel2ItemsCall < 150) {
            navidebug.log("Reject showLevel2Items because called too fast:", now - lastShowLevel2ItemsCall, "ms");
            return;
        }    
...
var initNaviplusApp = function (e) {
    ...
    /********************************************/
    // Lấy attribute từ uigenJsScriptElements (đã đc failover từ currentScript)
    /********************************************/    
    var shop = uigenJsScriptElements.getAttribute('shop');
    var embed_id = '';
    if(uigenJsScriptElements.getAttribute('embed_id') != null)
        embed_id = uigenJsScriptElements.getAttribute('embed_id');
    var multisite = '';
    if(uigenJsScriptElements.getAttribute('multisite') != null)
        multisite = uigenJsScriptElements.getAttribute('multisite'); 
    /********************************************/
    ...
    if(embed_id != '') {
        ...
        if (uigenJsScriptElements.getAttribute('not_sticky') != null)
            not_sticky = uigenJsScriptElements.getAttribute('not_sticky');
        if(embed_id != '') if(uigenJsScriptElements.getAttribute('embed_title') != null)
            embed_title = uigenJsScriptElements.getAttribute('embed_title');
        if(embed_id != '') if(uigenJsScriptElements.getAttribute('embed_is_full') != null)
            embed_is_full = uigenJsScriptElements.getAttribute('embed_is_full');
        if(embed_id != '') if(uigenJsScriptElements.getAttribute('embed_margin') != null)
            embed_margin = uigenJsScriptElements.getAttribute('embed_margin');
    ...
    navihelper.linkCSSToHead( "https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" );
    navihelper.linkCSSToHead( "https://static.naviplus.app/icons/remixicon-lastest/remixicon.css" );        
    ...
    function runNavimanFunc( data, shopinfo ) {
        ...
        if( navihelper.isNeedCartCount(data, section_setting) ) {
            naviman.asyncGetCart().then( function(result) {
                // navidebug.log("Cart count: " + result.item_count );
    ...
    function getJSONData(shopinfo) {
        ...
        // Lấy data từ json (có thể là all hoặc theo embed_id)
        fetch(api_url)
        .then((response) => {
        ...
        .catch(() => {
            const file = api_url.split("/").pop();
            const flovUrl = "https://flov.naviplus.app/data/json/" + file + "?v=" + shopinfo["update_version"];
        ...
        .catch((error) => {
            // Bắt mọi lỗi còn lại (network lỗi, JSON.parse lỗi ở cả primary và fallback)
    ...
    // Bước 1: Lấy shopinfo để lấy ra update_version
    // Bước 2: Dùng update_version để lấy data từ server (all.json hoặc embed_id.json)
    // Bước 3: Chạy app    
    navihelper.getShopInfo(shop).then((shopinfo) => {
        getJSONData(shopinfo);
    });    
