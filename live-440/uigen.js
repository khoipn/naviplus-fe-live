 ...
 * Nếu không có backgroundColor thì dùng rgba(0,0,0,alpha) — nền trong suốt theo opacity.
 */
Menu.Common.Level1.generateOpacity = function(setting, cssNaviPrefix, menuKind) {
    ...
    if (isHadValue(bgColor)) {
        ...
        addHtml += cssNaviPrefix + '{ background-color: rgba(0,0,0,' + alpha + ') } ';
    }
...
Menu.Section.fixCSS_SectionParent = function (naviman_app, naviItem, embed_id, section_setting, menuKind) {    
    ...
    if( session != null ) {
        ...
        session.style.zIndex = (menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_GRID']) ? 0 : 1;
...
Menu.Item.TabSlide.switchTabInContainer = function(container, tabIndex, noAnimation) {
    ...
    // Fade out items cũ → display:none
...
var generateCSS_UI_Level2 = function ( setting, cssNaviPrefix, naviman_appItem, dragdrop, section_setting, menuKind ) {
    ...
    if (isHadValue(setting['submenuIconSize'])) {
        addHtml += cssNaviPrefix + ' ul li.item ul li.child .icon i { font-size: ' + setting['submenuIconSize'] + 'px; } ';
...
var showLevel2Items = function (menuItem, menuKind, embed_id, event) {
    ...
    if (menuItem != null) {
        if (!is_showing) {            
            ...
            if (menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_MOBILE_MEGAMENU'] && window.innerWidth <= 768)
