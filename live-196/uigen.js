 ...
 * https://cdn.jsdelivr.net/gh/khoipn/naviplus-fe-live@main/live-196/uigen.min.js
 * THÌ LẤY PHIÊN BẢN THEO CÁI NÀY **/
// var uigenJsScriptUrl = "https://cdn.jsdelivr.net/gh/khoipn/naviplus-fe-live@main/live-196/uigen.min.js";
...
// console.log(uigenJsScriptUrl);
console.log(uigenJsScriptElements);
/********************************************/


/********************************************/
/*
// Bỏ đi vì uigenJsScriptUrl sẽ ko còn tốt nữa, vì phụ thuộc vào currentScript
var versionMatch = uigenJsScriptUrl.match(/\/([^\/]+)\/uigen\.min\.js$/);
if (versionMatch) {
    naviman_version = versionMatch[1];
    // navidebug.log(naviman_version);
} else {
    // navidebug.log("No version found in the URL");
} */
/********************************************/
...
if( _NAVIPLUS_VERSION != 'DEV' ) // Định nghĩ trong start.js 
    ...
    // Do nothing. Khi đó naviman_version sẽ là "no-version", _NAVIPLUS_VERSION = "DEV".
}    

// console.log("naviman_version", naviman_version);
// console.log(_NAVIPLUS_VERSION);
...
****************************************************************************************************************/    
    var drawBottomNav = function (response, naviplusAppUrl, var_shop, var_embed_id, section_setting = [], fixMenuKind = 0) {
        ...
        navidebug.log(`⏰ Time to drawBottomNav: ${performance.now() - window.debugTimeStart} ms`);
...
/** 1.Link CSS & icons **************************************/
    ...
    linkCSSToHead( "https://static.naviplus.app/icons/remixicon-lastest/remixicon.css" );        
    ...
    function runNaviman( data, shopinfo ) {
        ...
        navidebug.log(`⏰ Time to runNaviman: ${performance.now() - window.debugTimeStart} ms`);

        if( isNeedCartCount(data) ) {
            naviman.asyncGetCart().then( function(result) {
                // navidebug.log("Cart count: " + result.item_count );

                navidebug.log(`⏰ Time to runNaviman + isNeedCartCount: ${performance.now() - window.debugTimeStart} ms`);
    ...
    function runApplication(shopinfo) {
        ...
        if( shopinfo["update_version"] != jsonUpdateVersion ) {
            navidebug.log("There is a new version of Navi+ data! Loading from server...");
            clearNaviSectionStorage();
            isNeedRequestServer = true;
        }

        navidebug.log(`⏰ Time to runApplication: ${performance.now() - window.debugTimeStart} ms`);

        // TODO: Chỗ này có thể cải tiến tốc độ load file bằng cách lấy thông tin  chính xác về version.
        if (!window._naviVer) {
            window._naviVer = Date.now();
            navidebug.log( "Loaded on uigen: _naviVer = ", window._naviVer );
        }
        else 
        navidebug.log( "Loaded on theme: _naviVer variable is created! Don't need reload" );
        ...
        if( isNeedRequestServer ) {
            ...
            .catch(() => {
                ...
                const flovUrl = "https://flov.naviplus.app/data/json/" + file + "?v=" + window._naviVer;
            ...
            navidebug.log("[Session storage] " , api_url);
...
function waitForElement(selector, callback) {
  ...
  if (!targetNode) {
    document.addEventListener('DOMContentLoaded', () => waitForElement(selector, callback), { once: true });
    return;
  }
...
waitForElement('#naviman_app', () => {
  navidebug.log("%cStart naviman_runApp", "color: purple; font-weight: bold; font-size: 16px;");
  naviman_runApp();
});
