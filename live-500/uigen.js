 ...
 * Yêu cầu đồng thời: env === 'demo' VÀ domain nằm trong whitelist.
 *
...
DemoMarket.loadEmbeds = function(opts) {
    ...
    p.embedIds.forEach(function(eid) {
        var url = opts.cdnJsonBase + '/' + opts.standardize(opts.shop, opts.token) + '.' + eid + '.json?v=0';
...
/*
 * hexToRgba — Chuyển mã màu HEX sang chuỗi CSS rgba() với alpha tuỳ chỉnh.
 *
 * Hỗ trợ HEX 6-digit (#RRGGBB) và 8-digit (#RRGGBBAA).
 * Khi hex có AA byte → nhân với tham số alpha để giữ đúng ý định cả hai.
 * Ví dụ: backgroundColor = #FFFFFF80 (50%) + bgOpacity overlay = 0.3 → rgba(255,255,255, 0.15)
 *
 * @param {string} hex    Mã màu dạng #RRGGBB hoặc #RRGGBBAA (có hoặc không có dấu #)
 * @param {number} alpha  Giá trị alpha 0.0–1.0 (mặc định 1 = đục hoàn toàn)
...
var hexToRgba = function(hex, alpha = 1) {
     ...
     * Nhân với alpha param để giữ đúng ý định của cả hai lớp opacity. */
    let hexAlpha = 1;
    if (hex.length === 8) {
    ...
    /* Nhân hexAlpha × alpha param — giữ đúng cả hai lớp opacity */
    const finalAlpha = +(hexAlpha * alpha).toFixed(3);
...
var applyHamburgerWidthSettings = function ( data, embed_id ) {
    ...
    if (isHadValue(wMobile)) {
        ...
        css += `@media (max-width: ${Helper.Env.MOBILE_MAX}px) {`
    ...
    if (isHadValue(wDesktopMax)) {
        ...
        var _childrenPosCSS = _hamburgerPos === 2
            ? ' right: ' + wD + '; left: auto; '
            : ' left: ' + wD + '; ';
        css += `@media (min-width: ${Helper.Env.DESKTOP_MIN}px) {`
...
function formatAttributes(attrArray) {
    return attrArray.map(attrPair => {
        ...
        // Strip existing quotes from value before wrapping in double quotes
...
/**
 * Hiệu ứng beacon/ping — chấm đỏ cố định + sóng lan toả vô hạn ở góc trên-trái element.
 * Dùng trong demo mode để thu hút click vào CSS selector được gán làm trigger.
 * Có thể tái sử dụng cho các loại menu khác.
 * Gọi Animation.stopDemoTrigger(el) để dừng.
 *
 * @param {Element} el - Element cần đánh dấu
 */
Animation.flashDemoTrigger = function(el) {
    if (el._naviDemoBeacon) return; // Đã có rồi, không tạo thêm

    var pos = window.getComputedStyle(el).position;
    if (pos === 'static') el.style.position = 'relative';

    // CSS keyframes — inject 1 lần duy nhất vào head
    if (!document.getElementById('navi-demo-beacon-style')) {
        var style = document.createElement('style');
        style.id = 'navi-demo-beacon-style';
        style.textContent = [
            '@keyframes _navi_beacon_ping {',
            '  0%   { transform: scale(1); opacity: 0.8; }',
            '  100% { transform: scale(3); opacity: 0; }',
            '}',
            '@keyframes _navi_beacon_dot {',
            '  0%, 100% { opacity: 1; }',
            '  50%       { opacity: 0.4; }',
            '}',
        ].join('');
    ...
    ping.style.cssText = [
        ...
        'animation:_navi_beacon_ping 0.9s ease-out infinite',
    ].join(';');
    ...
    dot.style.cssText = [
        ...
        'animation:_navi_beacon_dot 0.9s ease-in-out infinite',
    ].join(';');

    el.appendChild(ping);
    el.appendChild(dot);
    el._naviDemoBeacon = { ping: ping, dot: dot };
};

/**
 * Dừng và xoá beacon khỏi element.
 *
 * @param {Element} el
 */
Animation.stopDemoTrigger = function(el) {
    if (!el._naviDemoBeacon) return;
    var b = el._naviDemoBeacon;
    if (b.ping && b.ping.parentNode) b.ping.parentNode.removeChild(b.ping);
    if (b.dot  && b.dot.parentNode)  b.dot.parentNode.removeChild(b.dot);
    delete el._naviDemoBeacon;
};
...
Helper.localizeUrlByCurrentLang = function(url, item) {
    ...
    if (isAbsolute) {
        try {
            ...
            var out = u.toString();
            console.log("[Navi+ i18n-link] absolute same-host: " + url + " => " + out);
            return out;
        } catch (e) {
            return url;
        }
    ...
    console.log("[Navi+ i18n-link] relative: " + url + " => " + localized);
    return localized;
};
...
Helper.waitToReplaceInsert = function(el, divNaviItem, mode = 'replace', timeout = 3000, interval = 100) {
  return new Promise((resolve, reject) => {
...
Helper.waitToReplaceInsertMulti = function(rawSelectors, divNaviItem, mode, timeout) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    function tryInsert(from) {
      ...
      for (var i = 0; i < selectors.length; i++) {
        var target = null;
        try { target = document.querySelector(selectors[i]); } catch(e) {
        ...
        if (target) {
          ...
          var newElement = Helper.getNewElementReplaceInsert(divNaviItem);
          Helper.deployReplaceInsertMode(mode, target, newElement);
          resolve({ target: target, waited: waited, selector: selectors[i] });
          return;
        }
      }
    }
    ...
    if (isSettingBeTrue(setting['level1BackgroundHide'])) { // Nếu không có nền thì bỏ nền và bỏ bóng.
        ...
        if (isHadValue(setting['backgroundImage'])) {
            if (setting['backgroundImage'].includes("?")) {
                ...
                /* Không có suffix ?opacity — vẫn phải áp ảnh nền */
                if (isHadValue(setting['backgroundColor']))
                    addHtml += cssNaviPrefix + '{ background: ' + setting['backgroundColor'] + ' } ';
                addHtml += cssNaviPrefix + '{ background-image: url("' + setting['backgroundImage'] + '"); background-size: cover; } ';
            }
...
Menu.Sticky.lockPageScrollingTabBar = function (menuKind, isLock, setting) {
    ...
    Menu.Sticky.fixCSS_ForDesktop = function (cssNaviPrefix, setting, dragdrop, isNaviSection, section_setting, menuKind) {
...
Menu.Context.checkTriggerHamburger = function(naviItem) {
    ...
    entries.forEach(function(entry) {
        ...
        /* Lọc theo thiết bị hiện tại */
        var isMobile = Helper.Env.isOnMobile();
        if (device === "M" && !isMobile) return;
        if (device === "D" && isMobile) return;
        ...
        appendSlideMenuTo(selector, embedId, color, boxed, side);
    });
};
...
Menu.Item.TabSlide.buildCSS = function(idCssNaviPrefix, setting) {
    ...
    // iconSize: kích thước icon trong tab button
    if (isHadValue(setting['iconSize'])) {
        css += idCssNaviPrefix + ' .tab-slide-tab-icon i { font-size: ' + setting['iconSize'] + 'px !important; } ';
        css += idCssNaviPrefix + ' .tab-slide-tab-icon img { width: ' + setting['iconSize'] + 'px !important; height: ' + setting['iconSize'] + 'px !important; } ';
    }
...
Menu.Mega._getElementsToPushZIndex = function (menuItem) {
    ...
    if (bodyChild && bodyChild !== menuItem && bodyChild.parentElement === document.body && targets.indexOf(bodyChild) < 0) {
        ...
        var _naviStickyOutside = false;
        var _naviStickies = document.querySelectorAll('.naviItem.STICKY_TABBAR, .naviItem.STICKY_FAB_SUPPORT');
        for (var _si = 0; _si < _naviStickies.length; _si++) {
            if (!bodyChild.contains(_naviStickies[_si])) { _naviStickyOutside = true; break; }
        }
        if (!_naviStickyOutside) targets.push(bodyChild);
    }
...
/*        addHtml += cssNaviPrefix + ' ul li.item .icon, ' + cssNaviPrefix + ' ul li.item .image { height: ' + (21 + (setting['spaceTextIcon'] - 2)) + 'px; } ';*/
    ...
    if (setting['desktopHoverStyle'] !== 'underline') {
        ...
        if (_bgColor || _txtColor || _txtAnim === 'none' || _txtAnim === 'bounce-x') {
            addHtml += '@media screen and (min-width:769px) { ';
...
var generateCSS = function(naviman_appItem, embed_id, setting, dragdrop, isNaviSection, section_setting, menuKind) {
    ...
    if( // Is sticky menu
        ...
        if (setting['desktopDisplay'] == "true" || setting['desktopDisplay'] == true)
            html += Menu.Sticky.fixCSS_ForDesktop(cssNaviPrefix, setting, dragdrop, isNaviSection, section_setting, menuKind);
    }
    ...
    var teardownRenderedMenus = function (embedIds) {
        ...
        list.forEach(function (it) {
            ...
            if (el) {
                var prev = el.previousElementSibling; // <style> sinh kèm nằm beforebegin
    ...
    children.forEach((child) => {
      ...
      */
      ...
      html += '<li ' +
          ...
          + '" ' + childOnClick + ' data-name="' + Helper.HTML.escapeDoubleQuotedAttribute(naviLanguage.stringByLanguage(child["name"])) + '" data-kind="' + Helper.HTML.escapeDoubleQuotedAttribute(String(childKind)) + '">';
    ...
    data["dragdrop"].forEach((item) => {
        ...
        */
        ...
        html += '<li ' +
            ...
            + '" ' + itemOnClick + ' data-name="' + Helper.HTML.escapeDoubleQuotedAttribute(naviLanguage.stringByLanguage(item["name"])) + '" data-kind="' + Helper.HTML.escapeDoubleQuotedAttribute(String(itemKind)) + '"' + 'data-fullexpand="'+  Helper.HTML.escapeDoubleQuotedAttribute(String(getExtVariable(item, "fullexpand", "0"))) +'"' + 'data-fullexpand-width="'+  Helper.HTML.escapeDoubleQuotedAttribute(String(getExtVariable(item, "fullexpandwidth", ""))) +'"' + 'data-tabend="' + Helper.HTML.escapeDoubleQuotedAttribute(String(getExtVariable(item, "tabend", ""))) + '"'
...
var getNaviActionClass = function ( url, itemKind, isParent ) {
    ...
    if( k == NAVIGLOBAL['ITEM_KINDS']['GROUP_TITLE'] || k == NAVIGLOBAL['ITEM_KINDS']['BLANK_SPACE'] )
        return "";
    if( isParent )
        return " navi-action ";
    if( url !== "" && url !== "#" )
...
var isItemSMS = function ( item_url ) {
    ...
    return  false;
        ...
        var callDrawLoop = function(
            ...
            const timer = setInterval(() => {
                ...
                if (!el) {
                    ...
                    if (isPublishToPlace && el_item && !el_item.isConnected) {
                        ...
                        if (_raw) {
                            ...
                            Helper.waitToReplaceInsertMulti(_raw, divNaviItem, _mode).then(function(r) {
        ...
        response.forEach((naviItem) => {
            ...
            isNaviSection = (embed_id != '' ) && !isStickyKind;
            ...
            if ( ["SECTION_MOBILE_HEADER", "SECTION_MOBILE_MEGAMENU", "SECTION_MOBILE_GRID", "SECTION_MOBILE_BANNER", "SECTION_DESKTOP_MEGAMENU"].includes(menuKindClass) ) {
                ...
                if (typeof DemoMarket !== 'undefined' && DemoMarket.isActive(section_setting['env'])) {
                    if (["SECTION_DESKTOP_MEGAMENU", "SECTION_MOBILE_MEGAMENU", "SECTION_MOBILE_GRID"].includes(menuKindClass)) {
                        ...
                        naviItem["data"]["setting"]["publishToPlace"]        = "header";
                        naviItem["data"]["setting"]["publishToPlaceKind"]    = "2";
                ...
                if( isPublishToPlace == true ) {
                    ...
                    (async () => {
                        try {
                            ...
                            var result = await Helper.waitToReplaceInsertMulti(publishToPlaceRaw, divNaviItem, insertMode);
...
var showLevel3Items_Desktop = function (menuItem, isNaviSection, menuKind, embed_id, event) {
    ...
    if (!isOnMobile) {
        ...
        if (menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU']) {
            ...
            ulChildrent.style.top = liRect.top + "px";
            ulChildrent.style.bottom = "auto";
            if (isRightDir) {
                ulChildrent.style.left = panelRect.right + "px";
                ulChildrent.style.right = "auto";
            } else {
                ulChildrent.style.left = "auto";
                ulChildrent.style.right = (window.innerWidth - panelRect.left) + "px";
            }
...
var adjustMenuPosition = function (menuItem, menuKind, embed_id) {
    ...
    /* Xóa transform và margin-left cũ trước khi đo để tránh tích lũy qua nhiều lần mở.
     * SECTION_DESKTOP_MEGAMENU dùng margin-left thay transform để tránh tạo containing block
     * cho position:fixed (xem bên dưới). */
    ...
    if (translateX !== 0) {
        /* SECTION_DESKTOP_MEGAMENU: dùng margin-left thay vì transform để tránh tạo CSS containing block
         * cho position:fixed. Nếu panel có transform, các con position:fixed sẽ bị đặt tương đối
         * theo panel (không phải viewport) và bị clip bởi overflow:hidden → level 3 bị ẩn.
         * margin-left đạt cùng hiệu quả căn giữa mà không tạo containing block. */
        // TASK00193: STICKY_TABBAR cũng dùng margin-left (không transform) vì L2 panel chứa
        // level 3 position:fixed — transform sẽ biến L2 thành containing block, giam L3 lại + clip.
        if (menuKind == NAVIGLOBAL['MENU_KINDS']['SECTION_DESKTOP_MEGAMENU'] ||
            menuKind == NAVIGLOBAL['MENU_KINDS']['STICKY_TABBAR']) {
            menuChildren.style.marginLeft = translateX + 'px';
        } else {
...
/***** Theme: Sleek https://themes.shopify.com/themes/sleek/presets/sleek ******************/
var openMenu_Sleek = function() {
    callbackPublicFunc_delay(openMenu_Sleek);
    return clickToElement('.menu-drawer-button');
};
var openSearch_Sleek = function() {
    callbackPublicFunc_delay(openSearch_Sleek);
    return clickToElement('.search-drawer-button');
};
var openCart_Sleek = function() {
    callbackPublicFunc_delay(openCart_Sleek);
    return clickToElement('.cart-drawer-button');
};
...
var resetShopifyInboxVisibilityForPlatform = function (hasMobile, hasDesktop) {
    ...
    if (Helper.Env.isOnMobile() && !hasMobile) shouldShow = true;
    if (!Helper.Env.isOnMobile() && !hasDesktop) shouldShow = true;
...
/*
 * appendSlideMenuTo(cssSelector, slideMenuEmbedID, color, is_boxed)
 * Tìm phần tử logo qua cssSelector, thêm class naviplus_slide_icon để hiện icon hamburger
 * bằng CSS ::before, rồi gán click → openNaviMenu(slideMenuEmbedID).
 *
 * - Nếu gọi sớm (logo chưa có trong DOM): tự chờ bằng MutationObserver, gắn đúng 1 lần khi xuất hiện.
 * - Idempotent: gọi nhiều lần cũng không bị trùng.
 * - CSS base inject 1 lần, CSS per-instance inject theo class riêng của (embedID + settings).
 *   Nhiều selector cùng menu có thể dùng boxed/color/side khác nhau mà không bị xung đột.
 *
 * Mỗi logo element nhận 2 class:
 *   naviplus_slide_icon                         — style chung (::before hamburger)
 *   naviplus_slide_icon_<embedID>_<settingsKey> — scope riêng cho màu, boxed, side
 *
 * @param {string}  cssSelector      - CSS selector trỏ đến phần tử chứa logo
 * @param {string}  slideMenuEmbedID - ID của .naviItem.CONTEXT_SLIDE (có hoặc không có # . đều được)
 * @param {string}  [color]          - Màu icon: "#FFF", "FFF", "#FAFAFA", "rgba(...)" — mặc định đen
 * @param {boolean} [is_boxed=false] - true: bọc icon trong box vuông 28×28, icon thu nhỏ 20×20 cách lề 4px
 * @param {string}  [side='left']    - Vị trí icon: "left" (mặc định) hoặc "right"
 *
 * -----------------------------------------------------------------------
 * Nếu KHÔNG muốn dùng hàm này (không cần event, chỉ cần icon hiển thị),
 * có thể copy CSS sau vào theme (thay .your-logo-selector bằng selector thực tế):
 *
 * Mode thường:
 *   .your-logo-selector {
 *     position: relative;
 ...
 *     color: #000; /* hoặc màu tùy chọn *\/
 *   }
 *   .your-logo-selector::before {
 *     content: '';
 *     display: block;
 *     position: absolute;
 *     left: 0;
 ...
 *                 -4px -9px 0 0 currentColor, 18px -9px 0 0 currentColor,
 *                 -4px 9px 0 0 currentColor,  18px 9px 0 0 currentColor,
 *                 -4px 0 0 0 currentColor,    18px 0 0 0 currentColor;
 *   }
 *   /* Cách đơn giản hơn: dùng outline + padding trên ::before *\/
 *
 * -----------------------------------------------------------------------
 */
var appendSlideMenuTo = function(cssSelector, slideMenuEmbedID, color, is_boxed, side) {
    slideMenuEmbedID = slideMenuEmbedID.replace(/^[#.]/, "");

    /* Chuẩn hóa color: "FFF" → "#FFF", undefined → null (dùng currentColor) */
    var iconColor = null;
    if (color && typeof color === "string" && color.trim() !== "") {
        iconColor = color.trim();
        if (iconColor.charAt(0) !== "#" && !iconColor.startsWith("rgb") && !iconColor.startsWith("hsl")) {
            iconColor = "#" + iconColor;
        }
    }

    var boxed = is_boxed === true;
    var align = (side === "right") ? "right" : "left";

    /* CSS base: inject 1 lần dùng chung — chỉ định vị trí, không định màu */
    var baseStyleId = "navi-hamburger-style";
    if (!document.getElementById(baseStyleId)) {
        ...
        baseEl.id = baseStyleId;
        baseEl.textContent = baseCss;
        document.head.appendChild(baseEl);
    }

    /* CSS per-instance: keyed theo settings (embedId + boxed + align + color)
     * để nhiều selector cùng menu có thể dùng style khác nhau mà không bị ghi đè. */
    var colorKey = iconColor ? iconColor.replace(/[^a-zA-Z0-9]/g, "") : "def";
    var settingsKey = slideMenuEmbedID + "_" + (boxed ? "b" : "n") + "_" + align + "_" + colorKey;
    var instanceStyleId = "navi-hamburger-style-" + settingsKey;
    if (!document.getElementById(instanceStyleId)) {
        var instanceClass = ".naviplus_slide_icon_" + settingsKey;
        var color_val = iconColor || "currentColor";

        var instanceCss;
        if (boxed) {
            /* Boxed: box 28×28, icon 14px rộng, 3 đường cách nhau 5px, cách lề 4px 4 phía */
            instanceCss = `
                ${instanceClass} {
                    padding-${align}: 36px !important;
                }
                ${instanceClass}::before {
                    ${align}: 0;
                    width: 28px;
                    height: 28px;
                    background: transparent;
                    outline: 2px solid ${color_val};
                    border-radius: 3px;
                    box-shadow: none;
                }
                ${instanceClass}::after {
                    ...
                    box-shadow: 0 5px 0 ${color_val}, 0 -5px 0 ${color_val};
                    transition: opacity 0.2s;
                }
            `;
        } else {
            /* Normal: icon 18px, 3 đường cách nhau 6px */
            instanceCss = `
                ${instanceClass} {
                    padding-${align}: 32px !important;
                }
                ${instanceClass}::before {
                    ${align}: 0;
                    width: 18px;
                    height: 2px;
                    background: ${color_val};
                    box-shadow: 0 6px 0 ${color_val}, 0 -6px 0 ${color_val};
                }
            `;
        }

        var instanceEl = document.createElement("style");
        instanceEl.id = instanceStyleId;
        instanceEl.textContent = instanceCss;
        document.head.appendChild(instanceEl);
    }

    /* Gắn class + event vào logo element — chỉ gắn 1 lần */
    var _bind = function(logoEl) {
        if (logoEl._naviHamburgerBound) return;
        logoEl._naviHamburgerBound = true;

        /* ::before không hoạt động trên replaced elements (img, input, video...).
         * Leo lên parentElement để gắn — đánh dấu img gốc để không gắn trùng lần sau. */
        var REPLACED = { IMG: 1, INPUT: 1, VIDEO: 1, IFRAME: 1, CANVAS: 1, EMBED: 1 };
        if (REPLACED[logoEl.tagName] && logoEl.parentElement) {
            navidebug.log("appendSlideMenuTo: " + logoEl.tagName + " là replaced element, gắn vào parentElement thay");
            logoEl = logoEl.parentElement;
            if (logoEl._naviHamburgerBound) return;
            logoEl._naviHamburgerBound = true;
        }

        logoEl.classList.add("naviplus_slide_icon");
        logoEl.classList.add("naviplus_slide_icon_" + settingsKey);

        logoEl.addEventListener("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            openNaviMenu(slideMenuEmbedID);
        });

        navidebug.log("appendSlideMenuTo: gắn vào", cssSelector, "→ mở #" + slideMenuEmbedID, { color: iconColor, boxed: boxed, side: align });
    };

    var logoEl = document.querySelector(cssSelector);
    if (logoEl) {
        _bind(logoEl);
        return;
    }

    /* Logo chưa có trong DOM — dùng MutationObserver để chờ */
    navidebug.log("appendSlideMenuTo: chờ selector", cssSelector);
...
// TASK00016. Begin
/* Đóng menu bằng phím ESC — chỉ áp dụng trên desktop (innerWidth > 768).
 *
 * Slide menu (CONTEXT_SLIDE):
 *   - slide-horizontal có level 3 → closeLevel3 (về level 2)
 *   - slide-horizontal có level 2 → closeLevel2 (về level 1)
 *   - chỉ level 1 hoặc menu khác → đóng toàn bộ
 *
 * Tabbar / FAB / Mega (non-slide):
 *   - level 3 mở → hideAllLevel3Items (về level 2)
 *   - level 2 mở (không có 3) → closeByOverlayGlobalForNonSlideMenus (về level 1)
 */
document.addEventListener("keydown", function(e) {
    if (e.key !== "Escape") return;
    if (Helper.Env.isOnMobile()) return;

    /* --- Slide menus (CONTEXT_SLIDE) --- */
    var slideClasses = [
        "hamburger-left-right",
        "hamburger-right-left",
        "hamburger-top-down",
        "hamburger-down-top",
        "hamburger-fullscreen",
        "hamburger-fullpopup"
    ];
    var selector = slideClasses.map(function(c) { return "." + c; }).join(",");
    var openSlides = document.querySelectorAll(selector);

    openSlides.forEach(function(element) {
        if (getComputedStyle(element).visibility !== "visible") return;
        e.preventDefault();

        if (element.getAttribute("ui") === "slide-horizontal") {
            var closedLevel = false;
            if (Menu.Context.Horizontal.getOpenedSubmenu(element, "3")) {
                navidebug.log("[ESC] slideH closeLevel3 | id=" + element.id);
                Menu.Context.Horizontal.closeLevel3(element);
                closedLevel = true;
            } else if (Menu.Context.Horizontal.getOpenedSubmenu(element, "2")) {
                navidebug.log("[ESC] slideH closeLevel2 | id=" + element.id);
                Menu.Context.Horizontal.closeLevel2(element);
                closedLevel = true;
            }
            if (closedLevel) return;
        }

        navidebug.log("[ESC] slide close menu | id=" + element.id);
        var _setting = (typeof getSettingOfNaviman === "function") ? getSettingOfNaviman(element.id) : {};
        var _dur = (typeof Animation !== "undefined" && Animation.getDuration)
            ? Animation.getDuration(_setting ? _setting["animationSpeed"] : undefined)
            : 300;
        Animation.hamburgerClose(element, {
            durationMs: _dur,
    ...
    /* --- Non-slide menus (Tabbar, FAB, Mega): kiểm tra level 3/2 ngoài CONTEXT_SLIDE --- */
    var nonSlideLevel3 = Array.from(document.querySelectorAll(".naviman_app li.menu-expand-level2"))
        .filter(function(li) { return !li.closest(".CONTEXT_SLIDE"); });
    var nonSlideLevel2 = Array.from(document.querySelectorAll(".naviman_app li.menu-expand-level1"))
        .filter(function(li) { return !li.closest(".CONTEXT_SLIDE"); });

    if (nonSlideLevel3.length > 0) {
        e.preventDefault();
        navidebug.log("[ESC] non-slide hideAllLevel3Items");
        naviman.hideAllLevel3Items();
    } else if (nonSlideLevel2.length > 0) {
        e.preventDefault();
        navidebug.log("[ESC] non-slide closeByOverlayGlobal");
        Finalizer.Event.closeByOverlayGlobalForNonSlideMenus();
    }

    /* Blur phần tử đang focus trong naviman để tránh browser hiện focus outline sau ESC */
    var _active = document.activeElement;
    if (_active && _active.closest && _active.closest(".naviman_app, .navi-hambuger-overlay-trigger, .navi-hambuger-overlay")) {
        _active.blur();
    }
});
// TASK00016. End
 ...
 *
 * 3. Burst mode (theo dõi theo tương tác người dùng — cùng hành vi bản production):
 *    - Không polling liên tục để tránh nặng web.
 *    - Chỉ chạy khi user click hoặc touch (listener trên document, capture).
 *    - Mỗi lần tương tác: burst tối đa 2 tick, mỗi tick cách 300ms.
 *    - Tick 1 chạy sau 300ms, tick 2 (nếu có) sau 600ms.
 *    - Nếu tick nào phát hiện floating active → dừng burst ngay.
 *
...
ZIndex._isNaviElement = function (el) {
    ...
    return !!el.closest(".naviItem, .naviman_app_overlay, .naviman_app_overlay_global, .section_naviman_app, .navi-hamburger-overlay, .navi-hambuger-overlay, .navi-hambuger-overlay-protect, .navi-hambuger-overlay-trigger");
};
...
ZIndex._applyFloatingActiveState = function (active) {
    if (active && !ZIndex._floatingLastActive) {
        var targetZ = 1;
        if (ZIndex._lastFloatingOverlayElement) {
            try {
                var fz = parseInt(window.getComputedStyle(ZIndex._lastFloatingOverlayElement).zIndex, 10);
                if (!isNaN(fz) && fz > 0) targetZ = Math.max(1, fz - 1);
            } catch (e) {}
        }
        navidebug.log("[ZIndex] floating detected → targetZ=" + targetZ + " (floating=" + (targetZ + 1) + "), el:", ZIndex._lastFloatingOverlayElement);
        ZIndex.downNaviMenuZIndex(targetZ);
    } else if (!active && ZIndex._floatingLastActive) {
        navidebug.log("[ZIndex] floating gone → restore");
...
/**
 * Gắn burst + click/touch capture để so khớp overlay theme với z-index Tabbar/FAB (xem đầu file).
 * initOverlayObserver chỉ gọi một lần/trang; vẫn giữ state burst trên ZIndex khi hàm chạy lại.
 */
...
/**
 * Hồi phục z-index của mọi element đã pushElementToMax, NGOẠI TRỪ những element
 * nằm trong danh sách `keepElements` (whitelist-by-identity).
 * Dùng cho closeDropdownsOfOtherMenus: đóng các menu khác nhưng giữ z-index
 * của ancestors thuộc menu hiện tại (đang mở level 2/3) — tránh bug
 * level 2 mega menu rơi xuống dưới khi mở level 3.
 *
 * @param {Element[]} keepElements - mảng element giữ nguyên trạng thái raised.
 */
ZIndex.restoreElementsFromMaxExcept = function (keepElements) {
    if (!ZIndex._pushedToMaxElements || !ZIndex._pushedToMaxElements.length) return;
    var keepSet = (keepElements && keepElements.length)
        ? (typeof Set !== "undefined" ? new Set(keepElements) : null)
        : null;
    var list = ZIndex._pushedToMaxElements.slice();
    var remaining = [];
    for (var i = 0; i < list.length; i++) {
        var el = list[i];
        var isKept = keepSet ? keepSet.has(el) : (keepElements && keepElements.indexOf(el) >= 0);
        if (isKept) { remaining.push(el); continue; }
        if (ZIndex._elementMaxStore && ZIndex._elementMaxStore.has(el)) {
            el.style.zIndex = ZIndex._elementMaxStore.get(el);
            ZIndex._elementMaxStore.delete(el);
        }
    }
    ZIndex._pushedToMaxElements = remaining;
};


/**
 * Giảm z-index của một element xuống targetZ (mặc định 1), lưu giá trị gốc để hồi phục.
 * Chỉ hạ nếu z-index hiện tại cao hơn targetZ; clamp targetZ tối thiểu = 1.
 */
ZIndex.pushElementToMin = function (element, targetZ) {
    if (!element || !element.style || !ZIndex._elementMinStore) return;
    if (ZIndex._elementMinStore.has(element)) return;
    var computed = window.getComputedStyle(element).zIndex;
    var currentZ = computed !== "auto" ? parseInt(computed, 10) : 0;
    targetZ = (typeof targetZ === "number" && !isNaN(targetZ)) ? Math.max(1, targetZ) : 1;
    if (currentZ <= targetZ) {
        navidebug.log("[ZIndex] pushElementToMin skip — currentZ=" + currentZ + " ≤ targetZ=" + targetZ, element);
        return;
    }
    navidebug.log("[ZIndex] pushElementToMin " + currentZ + " → " + targetZ, element);
    ZIndex._elementMinStore.set(element, computed !== "auto" ? computed : "0");
    ...
    function updatePosition() {
        ...
        if (parent) {
            ...
            if( insertMethod == "PublishToPlace" )
                dropdownFullExpand.style.top = Finalizer.Desktop.FullExpand.getFullExpandTop(dropdownFullExpand.parentElement);
            else
             ...
             * width="" (full screen) → left:0; width có giá trị → căn giữa viewport hoặc left:0 nếu rộng hơn viewport. */
            var elWidth = parseFloat(dropdownFullExpand.style.width) || dropdownFullExpand.offsetWidth;
            var vw = window.innerWidth;
            if (!elWidth || elWidth >= vw) {
                dropdownFullExpand.style.left = "0px";
                dropdownFullExpand.style.width = vw + "px";
            } else {
                dropdownFullExpand.style.left = Math.max(0, (vw - elWidth) / 2) + "px";
            }
    ...
    document.addEventListener("click", function(event) {
        ...
        if (
            ...
            if (
                ...
                /* Tránh mở lại level 2 khi DOM đã đóng (vd. sau open:Search / closeByOverlay) mà _opening* vẫn sót:
                 * showLevel2Items với is_showing=false sẽ vào nhánh *mở* và bật lại submenu. */
                var _hOpen = (typeof naviman !== "undefined" && naviman.Helper) || (typeof Helper !== "undefined" ? Helper : null);
                var subStillOpen = _hOpen && typeof _hOpen.isOpeningLevel2UlStillVisible === "function"
                    ? _hOpen.isOpeningLevel2UlStillVisible(window._openingMenuItem)
                    : false;
                if (!subStillOpen) {
                ...
                navidebug.log("[SlideMenu] handleTouchOutsideAllMenus → closeAllDropdowns | target=" + event.target.tagName + (event.target.className ? "." + String(event.target.className).trim().split(/\s+/)[0] : ""));
                if (naviman.Helper && typeof naviman.Helper.closeAllDropdowns === "function") {
                    naviman.Helper.closeAllDropdowns();
                }
            }
  ...
  const createDebugUI = () => {
    ...
    if (!document.getElementById(uiStyleId)) {
      ...
      style.textContent = `
        ...
        #${modalId} [data-part="find-menu-copy-prompt"]:hover { background: rgba(99,102,241,0.08); color: #4338ca; }
    ...
    headerHint.setAttribute("data-part", "header-hint");
    headerHint.appendChild(document.createTextNode("Enable: "));
    const hhc1 = document.createElement("code"); hhc1.textContent = TEXT.enableCode;
    headerHint.appendChild(hhc1);
    headerHint.appendChild(document.createTextNode("  ·  Disable: "));
    const hhc2 = document.createElement("code"); hhc2.textContent = TEXT.disableCode;
    headerHint.appendChild(hhc2);
    titleGroup.appendChild(title);
    titleGroup.appendChild(headerHint);
    ...
    if (document.getElementById("Details-menu-drawer-container")) {
      ...
      tip.innerHTML = ri("lightbulb-flash-line") + " Found <code>#Details-menu-drawer-container</code> — recommended for Slide Menu.";
    ...
    menuCardTitle.innerHTML = ri("sparkling-2-fill") + " Design menu by AI";
    ...
    menuScanBtn.innerHTML = ri("search-line", "Scan");
    ...
    const menuStep1Badge = document.createElement("span");
    menuStep1Badge.setAttribute("data-part", "find-menu-step-badge");
    menuStep1Badge.textContent = "1";
    const menuStep1Title = document.createElement("span");
    menuStep1Title.setAttribute("data-part", "find-menu-step-title");
    menuStep1Title.textContent = "Scan & select the menu to convert";
    menuStep1Header.appendChild(menuStep1Badge);
    ...
    menuStep1Desc.textContent = "Click Scan to detect menus on this page. Select the menu you want to convert to Navi+ to preview its structure.";
    ...
    menuOutputLabel.innerHTML = ri("file-list-3-line", "Menu Structure");
    ...
    menuCopyOutputBtn.innerHTML = ri("sparkling-line", "Generate to PROMPT");
    ...
    // ── Step 2 ──
    const menuStep2 = document.createElement("div");
    menuStep2.setAttribute("data-part", "find-menu-step");
    menuStep2.setAttribute("data-step", "2");
    menuStep2.style.display = "none";

    const menuStep2Header = document.createElement("div");
    menuStep2Header.setAttribute("data-part", "find-menu-step-header");
    const menuStep2Badge = document.createElement("span");
    menuStep2Badge.setAttribute("data-part", "find-menu-step-badge");
    menuStep2Badge.setAttribute("data-active", "");
    menuStep2Badge.textContent = "2";
    const menuStep2Title = document.createElement("span");
    menuStep2Title.setAttribute("data-part", "find-menu-step-title");
    menuStep2Title.textContent = "Generate PROMPT";
    menuStep2Header.appendChild(menuStep2Badge);
    menuStep2Header.appendChild(menuStep2Title);

    const menuStep2Desc = document.createElement("div");
    menuStep2Desc.setAttribute("data-part", "find-menu-step-desc");
    menuStep2Desc.textContent = "Your PROMPT is ready. Copy and paste it into any AI to redesign the menu structure.";

    const menuPromptWrap = document.createElement("div");
    menuPromptWrap.setAttribute("data-part", "find-menu-prompt");

    const menuPromptBar = document.createElement("div");
    menuPromptBar.setAttribute("data-part", "find-menu-prompt-bar");
    const menuPromptLabel = document.createElement("span");
    menuPromptLabel.innerHTML = ri("robot-line", "PROMPT");
    const menuCopyPromptBtn = document.createElement("button");
    menuCopyPromptBtn.type = "button";
    menuCopyPromptBtn.setAttribute("data-part", "find-menu-copy-prompt");
    menuCopyPromptBtn.innerHTML = ri("clipboard-line", "Copy");
    menuPromptBar.appendChild(menuPromptLabel);
    menuPromptBar.appendChild(menuCopyPromptBtn);

    const menuPromptText = document.createElement("textarea");
    menuPromptText.setAttribute("data-part", "find-menu-prompt-text");
    menuPromptText.readOnly = true;
    menuPromptText.spellcheck = false;

    menuPromptWrap.appendChild(menuPromptBar);
    menuPromptWrap.appendChild(menuPromptText);
    menuStep2.appendChild(menuStep2Header);
    menuStep2.appendChild(menuStep2Desc);
    menuStep2.appendChild(menuPromptWrap);
    menuCardBody.appendChild(menuStep2);

    // ── Step 3 ──
    const menuStep3 = document.createElement("div");
    menuStep3.setAttribute("data-part", "find-menu-step");
    menuStep3.setAttribute("data-step", "3");
    menuStep3.style.display = "none";

    const menuStep3Header = document.createElement("div");
    menuStep3Header.setAttribute("data-part", "find-menu-step-header");
    const menuStep3Badge = document.createElement("span");
    menuStep3Badge.setAttribute("data-part", "find-menu-step-badge");
    menuStep3Badge.textContent = "3";
    const menuStep3Title = document.createElement("span");
    menuStep3Title.setAttribute("data-part", "find-menu-step-title");
    menuStep3Title.textContent = "Paste to Navi+";
    menuStep3Header.appendChild(menuStep3Badge);
    menuStep3Header.appendChild(menuStep3Title);

    const menuStep3Desc = document.createElement("div");
    menuStep3Desc.setAttribute("data-part", "find-menu-step-desc");
    const menuStep3List = document.createElement("ol");
    menuStep3List.setAttribute("data-part", "find-menu-step3-list");
    [
      { t: "In Navi+ app, select the appropriate menu type (TABBAR, MEGA MENU, SLIDE MENU…)" },
      { t: " AI button in the menu editor", icon: "sparkling-line", pre: "Click the " },
      { t: "Paste the PROMPT into the chat box and click Run" }
    ].forEach(({ t, icon, pre }) => {
      const li = document.createElement("li");
      if (icon) li.innerHTML = (pre || "") + ri(icon) + t;
      else li.textContent = t;
      menuStep3List.appendChild(li);
    });
    menuStep3Desc.appendChild(menuStep3List);
    menuStep3.appendChild(menuStep3Header);
    menuStep3.appendChild(menuStep3Desc);
    menuCardBody.appendChild(menuStep3);
    ...
    menuCopyOutputBtn.addEventListener("click", () => {
      const fullPrompt = AI_PROMPT_PREFIX + menuOutputText.value;
      menuPromptText.value = fullPrompt;
      menuPromptText.style.height = "auto";
      menuPromptText.style.height = menuPromptText.scrollHeight + "px";
      menuStep2.style.display = "block";
      menuStep3.style.display = "block";
      menuStep2.scrollIntoView({ behavior: "smooth", block: "nearest" });
    });

    menuCopyPromptBtn.addEventListener("click", async () => {
      const ok = await dmCopyToClipboard(menuPromptText.value);
      if (ok) {
        menuCopyPromptBtn.innerHTML = ri("check-line", "Copied!");
        menuCopyPromptBtn.classList.add("dm-copied");
        window.setTimeout(() => {
          menuCopyPromptBtn.innerHTML = ri("clipboard-line", "Copy");
          menuCopyPromptBtn.classList.remove("dm-copied");
        }, 1600);
    ...
    const showMenuOutput = (menuEl) => {
      ...
      // Reset Step 2 when a different menu is selected
      menuStep2.style.display = "none";
      menuStep3.style.display = "none";
      menuPromptText.value = "";
    };
    ...
    menuScanBtn.addEventListener("click", () => {
      menuScanBtn.innerHTML = ri("loader-4-line dm-spin", "Scanning...");
      ...
      window.setTimeout(() => {
        ...
        menuScanBtn.innerHTML = ri("search-line", "Re-scan (" + menuLists.length + " found)");
...
var initNaviplusApp = function (e) {
    ...
    function renderData(json, shopinfo, needSwap) {
        if (needSwap) {
    ...
    async function getJSONData(shopinfo, token, needSwap) {
        ...
        if (cachedJson) {
            ...
            renderData(cachedJson, shopinfo, needSwap);
            return;
        }
        ...
        try {
            ...
            // Bước 6: Chạy app
            renderData(json, shopinfo, needSwap);
        } catch (error) {
    ...
    if (typeof DemoMarket !== 'undefined' && DemoMarket.isActive(deploy_env)) {
        ...
        (async function () {
            ...
            if (lastVersion) {
                ...
                if (cachedJson) {
                    runNavimanFunc(cachedJson, null);
                    drewFromCache = true;
                }
            }
