    ...
    data["dragdrop"].forEach((item) => {
        ...
        if (desktopHover) {
            if (!(itemExtClassname.includes('navi-nohover') || itemExtClassname.includes('navi-hover'))) {
                ...
                if (itemIsParent ) {
                    var itemMouseLeaveInner = 'clearTimeout(naviman.hoverTimeout); naviman.hoverTimeout = setTimeout(() => { if (!this.matches(\':hover\')) { this.classList.remove(\'navi-hover-active\'); ' + onhoverFunc + '; } }, '+ itemHoverTimeout +');';
